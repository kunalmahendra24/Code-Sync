...
const { setSocketIO } = require("./controllers/interviewRoomController");
const { setSocketIO: setExecutionSocketIO } = require("./controllers/executionController");
...
interviewRoomService
  ...
  .then((repaired) => {
    if (repaired > 0) {
      console.log(`Repaired ${repaired} orphaned interview space(s)`.yellow);
    }
  })
  .catch(console.log);
...
app.use("/interviews", require("./routes/interviewRoomRoutes"));
app.use("/questions", require("./routes/questionRoutes"));
app.use("/execute", require("./routes/executionRoutes"));
...
setSocketIO(io);
setExecutionSocketIO(io);

timerService.resumeActiveTimers().catch(console.log);
...
io.on("connection", (socket) => {
  ...
  registerInterviewSocketHandlers(io, socket);
  ...
  socket.on(ACTIONS.CODE_CHANGE, async ({ spaceId, change }) => {
    ...
    const allowed = await validateInterviewSocketAction(
      socket,
      "write_code"
    );
    if (!allowed) return;
